...
# Routing
Read `agent/task-routing.md`, load `agent/skills/adding-features/SKILL.md`, and
...
test manifest per `agent/testing-policy.md`.
...
2. Run the formatter (if configured), then narrow checks (frontend
   `npm run type-check`; relevant backend `pytest`), then `./scripts/check.sh`.
