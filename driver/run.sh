...
#
# Usage:
#   bash driver/run.sh                 # all tasks in order
#   bash driver/run.sh --task 03       # one task
#   bash driver/run.sh --from 02       # from task 02 onward
#   bash driver/run.sh --resume        # skip committed, resume current
#   bash driver/run.sh --status        # print status table and exit
#   bash driver/run.sh --selftest      # control-flow self-test (use with DRIVER_MOCK=1)
set -u
